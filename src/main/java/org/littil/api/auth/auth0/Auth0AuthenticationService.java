...
import org.littil.api.auth.AuthenticationService;
...
public class Auth0AuthenticationService implements AuthenticationService {
	...
	public void assignRole(String userId, String roleName) {
		try {
			...
			Optional<String> roleId = getRoles().stream().filter(r -> r.getRole().equals(roleName))
					.map(RoleEntity::getId).findFirst();
			if (roleId.isPresent()) {
